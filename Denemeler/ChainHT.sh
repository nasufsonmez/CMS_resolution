...
rm run_card_default.dat

scp ${mg5}/RunCards/run_card.dat ${mg5}/$say/Cards/run_card_default.dat	



sed  -i 's/ 1.0   = htjmin ! minimum jet HT=Sum(jet pt)/ '${ht[$counter]}'   = htjmin ! minimum jet HT=Sum(jet pt)/1' run_card_default.dat
sed  -i 's/ -1.0  = htjmax ! maximum jet HT=Sum(jet pt)/ '${htt[$counter]}'  = htjmax ! maximum jet HT=Sum(jet pt)/1' run_card_default.dat
