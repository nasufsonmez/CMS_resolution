...
int main(int argc, char*argv[])
{
 ...
 /////////////////////// Book histograms ///////////////////////////////
    ...
    char hist_title[1000];
     ...
     TDirectory *ptResoNew = outf->mkdir("JetRelativePtResolutionPTBins_1");
     ptResoNew->cd();
        ...
        {
            sprintf(hist_name,"pt_Reso_PT%ito%i_1",ptIntervals[a],ptIntervals[a+1]);
            sprintf(hist_title,"%i < Pt < %i _1",ptIntervals[a],ptIntervals[a+1]);
            histRelativePtResoPTBins[a]          = new TH1F(hist_name,hist_title,100, 0.0, 2.0);
        }

     // PT RESO NEW METHOD Cambridge/Aachen  ------------------------------------------------------------

     TDirectory *ptResoNew_CA = outf->mkdir("JetRelativePtResolutionPTBins_2");
     ptResoNew_CA->cd();
        ...
        for ( int a=0; a < ptIntervalsArraySize-1; a++ )
        {
            sprintf(hist_name,"pt_Reso_PT%ito%i_2",ptIntervals[a],ptIntervals[a+1]);
            sprintf(hist_title,"%i < Pt < %i _2",ptIntervals[a],ptIntervals[a+1]);
...
////////////////// PT RESOLUTION NEW METHOD---------------------------------------------------------------
        ...
        if( genjetta.size() == jetta.size() )
        {
           for(unsigned i = 0 ; i < genjetta.size() ; ++i )
      	  {
              if ( ( genjetta[i]->Eta <= JETETACUT ) && ( genjetta[i]->PT > JETPTCUT ) )
              {
                 double delta_phi_for_Reso = abs(jetta[i]->Phi - genjetta[i]->Phi) ;
...
////////////////// PT RESOLUTION NEW METHOD Cambridge/Aachen   ---------------------------------------------------------------
        ...
        if( genjetta_CA.size() == jetta_CA.size() )
        {
           for(unsigned i = 0 ; i < genjetta_CA.size() ; ++i )
      	  {
              if ( ( genjetta_CA[i]->Eta <= JETETACUT ) && ( genjetta_CA[i]->PT > JETPTCUT ) )
              {
                 double delta_phi_for_Reso_CA = abs(jetta_CA[i]->Phi - genjetta_CA[i]->Phi) ;
...
///////////////// SCALAR HT  Cambrigde/Aachen --------------------------------------------------------
    ...
    cout << "! For inputfile " << inputFile << "..." << endl;
    cout << "! number of total entries : "<< numberOfEntries << endl;
    cout << "! ----------------------------------------------------------------------\n" ;
